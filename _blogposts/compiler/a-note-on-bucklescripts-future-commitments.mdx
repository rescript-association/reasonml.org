...
No. This new syntax is purely additive. It sits beside the existing OCaml and Reason syntax inside the BuckleScript compiler. We **won't** remove OCaml and Reason support from the BuckleScript compiler for a long time, whatever the surface emphasis might be.

**Will I be able to continue writing OCaml/Reason and compile to JS in the future?**

It follows from our previous guarantee that yes, you will still be able to. If the BuckleScript compiler _ever_ starts removing language features, it'll be on the `.res` syntax level; this means that you can just keep writing in OCaml and Reason syntax to access the complete set of OCaml features that BuckleScript supports today.
...
Consider also that with the release of this new syntax, people's entire toolchain, from the BuckleScript compiler to syntax to ReasonReact to editor tooling, has now become primarily community-owned, with much less reliance on the company (while retaining the same energy devoted to said projects by the company).
