import { Prose_make as Layout } from "../layouts/BeltDocsLayout.gen.js";
...
OCaml standard library or from the Js namespace, e.g. use `Belt.List` instead of
`List` or `Js.List` and `Belt.Array` instead of `Array` or `Js.Array`.

## Motivation

Belt improves over the OCaml standard library as follows:

- Consistent naming convention familiar to JS developers:
  - camlCase
  - arguments order (see section "Pipe First" below)  
- Exceptions:
  - Only functions suffixed with "Exn" will throw exceptions.
  - Other functions with an optional result will just return `option('a)`.
...
into your `bsconfig.json` instead.
