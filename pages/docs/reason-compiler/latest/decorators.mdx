...
`bs.deriving accessors` makes it more convenient to access records.
...
`bs.deriving abstract` makes it more convenient to access records.
...
If you want to use a function which needs to bind to a JS object's `this` context, you need to use `bs.meth`, to avoid issues with currying.

For instance, take the following JS object:

```js
var john = { 
  say: (a, b) => console.log(a, b)
};
```
