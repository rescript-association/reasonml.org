...
Reason records were represented as arrays in JS in BuckleScript versions < v7. `bs.deriving accessors` makes it more convenient to access records when you or your library consumers need to be on an older BuckleScript version.

<Info>

As records are compiled to JS objects since BuckleScript v7, you may not need this decorator anymore, except if you need to stay compatible for users with older BuckleScript versions (such as library authors).
Since [records](common-data-types.md#records) are a shared data type, it is pretty safe (and much more straightforward) to use them directly. 
However, if you have a lot of optional fields, it is still recommended to use `bs.deriving accessors`, since in records you need to explicitly initialize every field whereas `bs.deriving accessors` gives
you a record creation function where you can omit every optional argument.
...
Reason records were represented as arrays in JS in BuckleScript versions < v7. `bs.deriving abstract` makes it more convenient to access records when you or your library consumers need to be on an older BuckleScript version.

<Info>

As records are compiled to JS objects since BuckleScript v7, you may not need this decorator anymore, except if you need to stay compatible for users with older BuckleScript versions (such as library authors).
Since [records](common-data-types.md#records) are a shared data type, it is pretty safe (and much more straightforward) to use them directly.
However, if you have a lot of optional fields, it is still recommended to use `bs.deriving abstract`, since in records you need to explicitly initialize every field whereas `bs.deriving abstract` gives
you a record creation function where you can omit every optional argument.
...
`bs.as` can be used with [`bs.deriving abstract`](#bsderiving-abstract) or records (starting with BuckleScript v7) to rename fields. This is useful when field names are valid in JS, but not in BS.
...
To call a method of a `Js.t` field, use `bs.meth` to avoid issues with a curried object where the reference to `this` may be incorrect.
