...
**Hint:** Since BuckleScript v7, records are compiled to plain JS objects, so you might not need `[@bs.deriving accessors]`.
More about this [here](/docs/reason-compiler/latest/object#records-as-objects).

</Info>

The `[@bs.deriving accessors]` can be used to either generate getter functions for a decorated **record type**, or to generate creator functions for **variant type constructors**.
...
`bs.deriving abstract` is used to convert a **record type definition** into the following functions:

- A creator function, where labeled arguments represent the attributes of the record
- An abstract type representing the "abstract record"
- A set of setter / getter functions for each record attribute

When using this decorator, the original record type will not be usable as a Reason record anymore since it will be replaced by the generated functions.


For example an annotated record type `person`...
...
...would expand to the following code (we only describe the signatures of the generated functions, the implementation is hidden, that's why it's called "abstract"):
...
/* Getters */
let nameGet: (~name: string, ~age: int) => person;
let ageGet: (person) => int;

/* Setters */
let nameSet: (person, string) => person;
let ageSet: (person, int) => person;
```
...
If we would like to create a `person` type, we would now use the creation function:
...
Will retrieve the value of an abstract type attribute.
Continuing the previous example:

```reason
Js.log(nameGet(misha)); /* => "Misha" */
Js.log(ageGet(misha));  /* => 18 */
```
...
the corresponding values get inlined directly in the JS code:
