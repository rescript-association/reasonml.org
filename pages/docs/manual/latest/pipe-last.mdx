...
maybeGreeting |> Belt.Option.getExn -> Js.log;
```

will not compile, since the compiler will process the `->` pipe first (`Belt.Option.getExn -> Js.log`)
before applying `maybeGreeting` to `Belt.Option.Exn`. A fix for the example above would involve
